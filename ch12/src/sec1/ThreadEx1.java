package sec1;
//쓰레드(Thread) : 현재 실행 중인 프로그램 또는 데이터나 프로세스를 의미하여,
//실제 컴퓨터에서는 두 개 이상의 쓰레드가 실행 중이게 되는데 이를 멀티 쓰레드라 함.
//쓰레드를 시작하려면  Thread클래스 또는 ThreadRunnable 인터페이스로 부터 상속 받아 run 메소드를 기술하고,
//run 메소드 안에는 작업할 내용을 기술하ㅏ며, main() 함수에서 해당 객체를 생성 후 객체.run() 하면 쓰레드를 시작할 수 있다.
...
public class ThreadEx1 {
	...
	public static void main(String[] args) {
		//ThreadWithRunnable(인터페이스)로 상속받아 오버라이딩
		...
		Thread th3 = new Thread(new ThreadWithRunnable()); 	//비선점형
		th3.start();
...
class ThreadWithRunnable implements Runnable {
	...
	public void run() {
		for(int i=0;i<10;i++){
			...
			try {
				Thread.sleep(10);	//0.01초의 딜레이. 너무빨리실행되기때문에
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		}
